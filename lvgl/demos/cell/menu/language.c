...
static bool handle_click_event(const click_e click) {
  ...
  case CLICK_SHORT_UP:
    ...
    ui_helpers_calculate_y_positions(LANGUAGE_COUNT, screen->elements);
    break;
...
static void open_window() {
  ...
  const char* text[LANGUAGE_COUNT] = {"中文", "English"};
  ...
  ui_helpers_calculate_y_positions(LANGUAGE_COUNT, elements);
}
