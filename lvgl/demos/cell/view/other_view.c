...
void OtherViewCreate(OtherView *view) {
  ...
  view->trip_key_position =
      CreateLabelPos(31, 234, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"小计里程"});
  view->trip_value_position =
      CreateLabelPos(142, 235, 100, 25, color, kMicrosoftYaHei_20,
                     kTextInt, (LabelValue)9999);
  view->trip_unit_position =
      CreateLabelPos(180, 235, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"km"});
  view->avg_key_position =
      CreateLabelPos(31, 273, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"平均车速"});
  view->avg_value_position =
      CreateLabelPos(142, 272, 100, 25, color, kMicrosoftYaHei_20,
                     kTextInt, (LabelValue)58);
  view->avg_unit_position =
      CreateLabelPos(180, 272, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"km"});
  view->ride_time_key_position =
      CreateLabelPos(31, 312, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"骑行时间"});
  view->ride_time_value_position =
      CreateLabelPos(142, 311, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"00:15"});
  view->ride_time_unit_position =
      CreateLabelPos(180, 311, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"h"});
  view->battery_key_position =
      CreateLabelPos(31, 352, 100, 25, color, kMicrosoftYaHei_20,
                     kTextChar, (LabelValue){"电池电压"});
  view->battery_value_position =
      CreateLabelPos(142, 351, 100, 25, color, kMicrosoftYaHei_20,
                     kTextFloat, (LabelValue)12.0);
  view->battery_unit_position =
      CreateLabelPos(180, 351, 100, 25, color, kMicrosoftYaHei_20,
