...
void ui_helpers_calculate_y_positions(int count, lv_obj_t** element) {
  const int screen_height = 480;
  const int selected_font_size = 39;
  const int normal_font_size = 27;
  const int gap = 35;
  ...
  int total_height =
      (normal_font_size + gap) * (count - 1) + selected_font_size;

  int start_y = (screen_height - total_height) / 2;

  const int size_difference = selected_font_size - normal_font_size;
  for (int i = 0; i < count; i++) {
    int font_size =
        (i == selected_index) ? selected_font_size : normal_font_size;
    int y_position = start_y + (i * (normal_font_size + gap));

    if (i > selected_index) {
      y_position += size_difference;
    }
    lv_obj_set_y(elements[i], y_position);
    if (i == selected_index)
      lv_obj_set_style_text_font(elements[i], &HarmonyOS_Sans_SC_36, 0);
    else
      lv_obj_set_style_text_font(elements[i], &HarmonyOS_Sans_SC_24, 0);
  }
}
