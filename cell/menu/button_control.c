...
void button_control_init() {
  ...
  for (size_t i = 0; i < sizeof(image_names) / sizeof(image_names[0]); ++i) {
    for (size_t j = 0; j < sizeof(image_states) / sizeof(image_states[0]); ++j) {
...
void button_control_toggle_display() {
  ...
  for (uint8_t i = 0; i < sizeof(image_names) / sizeof(image_names[0]); ++i) {
    for (uint8_t j = 0; j < sizeof(image_states) / sizeof(image_states[0]); ++j) {
      snprintf(image_paths[i * 2 + j], sizeof(image_paths[i * 2 + j]),
               RES_PRFIX "menu/%s/%s_%s.png", theme_suffix, image_names[i],
               image_states[j]);
    }
