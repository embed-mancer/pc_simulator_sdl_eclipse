...
void guage_view_create(guage_view_t *view) {
  ...
  if (view->mode == GUAGE_VIEW_MODE_BLOCK) {
    ...
    ui_helpers_create_image(view->background, &view->icon, view->icon_position);
    ...
    ui_helpers_create_label(view->background, &view->label[0],
                            view->label_position[0]);
    ui_helpers_create_label(view->background, &view->label[1],
...
void guage_view_main_oil(guage_view_t *view, lv_obj_t *background) {
  ...
  view->icon_position =
      ui_helpers_init_image_position(view->icon_position.image, 30, 430);
  view->line_position =
      ui_helpers_init_image_position(view->line_position.image, 91, 444);
  view->block_position[0] =
      ui_helpers_init_image_position(view->block_position[0].image, 92, 444);

  view->label_position[0] = ui_helpers_init_label_position(
  ...
  view->label_position[1] = ui_helpers_init_label_position(
      298, 440, 10, 20, color, LABEL_FONT_SOURCEHANSANSCN_18, VALUE_TYPE_CHAR,
      (label_value_t){"F"});
...
void guage_view_main_water(guage_view_t *view, lv_obj_t *background) {
  ...
  view->icon_position =
      ui_helpers_init_image_position(view->icon_position.image, 487, 430);
  view->line_position =
      ui_helpers_init_image_position(view->line_position.image, 554, 444);
  view->block_position[0] =
      ui_helpers_init_image_position(view->block_position[0].image, 554, 444);
  ...
  label_color_e color     = tool_get_color_base();
  view->label_position[1] = ui_helpers_init_label_position(
      531, 441, 10, 20, color, LABEL_FONT_SOURCEHANSANSCN_18, VALUE_TYPE_CHAR,
      (label_value_t){"C"});
  guage_view_create(view);
}
